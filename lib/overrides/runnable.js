...
Runnable.prototype.run = function(fn) {
  ...
  try {
    ...
    var args = [this._nightwatch.api(), done];
    this.resetTimeout(); // explicit async with `done` argument

    // TODO: support sync hooks
    var fnResult = this.fn.apply(ctx, args);
    
    if (this.type == 'hook' && this.title == '"before each" hook') {
      return; // don't restart the queue for before each
    }
    if (this.type == 'test') {
    ...
    } else if (this.type === 'test' && fnResult && typeof fnResult.then === 'function') {
      fnResult
        .then(function(){
          // console.log('MANUAL START', self._nightwatch.shouldRestartQueue())
